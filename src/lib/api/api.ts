...
import { SentryNearEarthObject } from "../entities/sentry";
...
export const NEOService = {
    ...
    getSentryById: async function getSentryById(id: string) {
        const url = API_URL + `/neo/sentry/${id}`;
        const response: SentryNearEarthObject = await requestData(url);
...
export function parseDate(dateString: string) {
    ...
    if ((date.getDate() <= 0 || Number.isNaN(date.getDate())) || 
